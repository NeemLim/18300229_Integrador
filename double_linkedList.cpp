...
{
	struct Node //Creates our nodes.
	{
	public:
		int data; //Holds a generic data value.
		Node* nextLink,
...
public:
	...
	void addBeginning(T data) //Adds an element to the list.
	{
		Node* newNode;
		newNode = new Node();
		newNode->data = data;
		...
		{
			beginning = newNode;
			beginning->prevLink = nullptr;
		}
		else
		{
			beginning->prevLink = newNode;
			newNode->nextLink = beginning;
	...
	void agregarDato(T n) {
		Node* nuevo = new Node;

		nuevo->data = n;
		nuevo->prevLink = nullptr;
		nuevo->nextLink = nullptr;

		if (beginning == nullptr) {
			beginning = nuevo;
		}
		else {
			beginning->prevLink = nuevo;
			beginning->prevLink->nextLink = beginning;
			beginning = nuevo;
		}
	}
	...
	void deleteSpecific(T valueToDelete) //Deletes a particular element in the list.
	{
		...
		{
			Node* erase = beginning,	//Element about to be erased
			...
			else
			{
				...
				if (erase->nextLink != nullptr)	//If not last
				{
	...
	void deleteEverything() //Clears list.
	{
		...
		while (beginning) //Deletes from first to last while nextLink not null.
		{
			erase = beginning;
			beginning = beginning->nextLink;
			delete erase;
		...
		case '1':	//Add
			...
			for (int i = 0; (GetAsyncKeyState(VK_LCONTROL)) == false and i < 100; i++)
			{
				if ((GetAsyncKeyState(VK_LCONTROL)) == false)
				{
					...
					cin >> dataValue;
					myCollection.addBeginning(dataValue);
				}
